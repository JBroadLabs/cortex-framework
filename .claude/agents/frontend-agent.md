---
name: frontend-agent
description: Frontend implementation specialist for React/TypeScript UI components and client-side logic. Invoked by Hub for frontend implementation work.
...
1.  **Read Story File (MANDATORY FIRST STEP)**:
    ...
    **Extract from story file**:
    - **Task Type**: From header (e.g., "Component Development")
    - **Module**: From header (Frontend)
    - **Phase**: Should be [I]
...
2.  **Load Context via Task Type**:
    ...
    ```python
    # Read the frontend index
    Read("docs/frontend/index.md")

    # Find "Context Loading by Task Type" section
    # Locate your Task Type (e.g., "Component Development")
    # Load ONLY the documents listed for that Task Type
    ```

    **Example**: If Task Type is "Component Development", index might specify:
...
3.  **Load Additional Context**:
    ...
    ```python
    # If story specifies additional docs, load each one
...
- **Knowledge**:
  ...
  - `docs/frontend/index.md` - Primary frontend context entrypoint
